...
if (isset($_GET['id'])){
  ...
  $IPADD=crypt ($ip_address, "ee");
    ...
    if($panierC->CheckProduitPanier($_GET['id'],$_GET['type'])==0)
    {
      if(isset($_GET['qty']))
      {
        $nouv_prix = $_GET['prix'] * $_GET['qty'];
        $pan=new Panier($IPADD,$_GET['id'],$_GET['qty'],$nouv_prix,$_GET['type']);
    ...
    {
      foreach($panierC->getPanierItem($_GET['id'],$_GET['type']) as $row){
       ...
       $pan=new Panier($IPADD,$_GET['id'],$quantite,$prix_total,$_GET['type']);
