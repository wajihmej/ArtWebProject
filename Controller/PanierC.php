...
class PanierC{
    ...
    function rechercherTicket($str){
        $sql="select * from Panier where categorie like '%".$str."%' or id_prod like '%".$str."%'";
...
public function afficherImageProductPaniers($type,$id){
...
public function getPanierItem($id,$type){


    $IPADD=crypt ($this->getRealIpUser(), "ee");

    $sql="SELECT * From Panier where id_utilisateur = '$IPADD' and id_prod=$id and type ='$type'";
    $db=config::getConnexion();
    try{
    $liste=$db->query($sql);
    return $liste;
...
public function afficherPanierWithNew($quantite,$id,$prix){
    ...
    $IPADD=crypt ($this->getRealIpUser(), "ee");
        ...
        $sql="UPDATE Panier SET quatite=:quatite,prix_total=:prix_total WHERE  id_utilisateur = '$IPADD' and id_prod=:id_prod and type =:type";
...
try{        
        ...
        $type=$Panier->getType();
        $datas = array(':id_prod'=>$id_prod,':quatite'=>$quatite,':type'=>$type);
...
public function ajouterPanier($Panier){
    $sql="insert into Panier(id_utilisateur,id_prod,quatite,prix_total,type) values(:id_utilisateur,:id_prod,:quatite,:prix_total,:type)";
    $db=config::getConnexion();
    try{
    $req=$db->prepare($sql);
        $id_utilisateur=$Panier->getId_utilisateur();
...
function CheckProduitPanier($id,$type){
    $IPADD=crypt ($this->getRealIpUser(), "ee");
