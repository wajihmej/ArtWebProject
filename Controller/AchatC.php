...
class AchatC{  
    ...
    function rechercherTicket($str){
        $sql="select * from Achat where categorie like '%".$str."%' or id_prod like '%".$str."%'";
...
public function afficherImageProductAchats($type,$id){
...
public function triachat($id){
    ...
    function modifierAchat($Achat,$id){
        $sql="UPDATE Achat SET id_utilisateur=:id_utilisateur,id_prod=:id_prod,quatite=:quatite,prix_total=:prix_total,type=:type WHERE id=:id";
...
try{        
        ...
        $type=$Achat->getType();
        $datas = array(':id'=>$id, ':id_utilisateur'=>$id_utilisateur,':id_prod'=>$id_prod,':quatite'=>$quatite,':type'=>$type);
        $req->bindValue(':id',$id);
...
public function ajouterAchat($Achat){
    $sql="insert into Achat(id_utilisateur,id_prod,quatite,prix_total,type) values(:id_utilisateur,:id_prod,:quatite,:prix_total,:type)";
    $db=config::getConnexion();
    try{
    $req=$db->prepare($sql);
        $id_utilisateur=$Achat->getId_utilisateur();
